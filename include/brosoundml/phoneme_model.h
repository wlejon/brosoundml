...
public:
    ...
    // Streaming. new_feats: (n_mels, N). out: (N, K). Maintains the per-conv
    // causal time-ring caches so a sequence of calls matches forward() over the
    // concatenation (within FP rounding). The head is pointwise in time, so no
    // ring/warm-up is needed beyond the trunk caches.
